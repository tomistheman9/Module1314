...
const SpiceGrill = ({ navigation, userDetails }) => {
    ...
    const fetchRestaurantAndDishes = async () => {
      try {
        ...
        if (restaurantResponse.ok) {
          ...
          const restaurant = restaurantData.find(rest => rest.id === 2);
        ...
        // Fetch dishes
        const dishesResponse = await fetch(`${NGROK_URL}/api/products?restaurant=2`);
